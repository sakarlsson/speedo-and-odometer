...
X27Stepper motor1(X12_STEP_A, X12_DIR_A);
...
void setup() {
	...
	pinMode(X12_RESET, OUTPUT);
	digitalWrite(X12_RESET, LOW);
	delayMicroseconds(1000);
	digitalWrite(X12_RESET, HIGH);
...
void loop() {
	...
	while (1) {
		int i = get_command();
		if (i > 0) {
      if (strcmp(cmd[0], "zero") == 0) {
				speedo_init();
      } else if (strcmp(cmd[0], "moveto") == 0) {
				mode = MOVETO_MODE;
